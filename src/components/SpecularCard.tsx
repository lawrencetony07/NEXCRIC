...
const SpecularCard = ({
  ...
  tintOpacity = 0.02,
  blur = 0,
  lineColor = '#10b981', // turf green highlight
  baseColor = '#1e293b', // slate
  intensity = 1.2,
  shineSize = 15,
  shineFade = 35,
  thickness = 1.5,
  ...
  return (
    <div
      ref={containerRef}
      className={`relative rounded-[var(--sc-radius)] [backdrop-filter:blur(var(--sc-blur))] shadow-sm transition-all duration-300 ${className}`}
      style={
        {
          '--sc-radius': `${radius}px`,
          '--sc-blur': `${blur}px`,
          background: `color-mix(in_srgb, ${tint} calc(${tintOpacity} * 100%), transparent)`
        } as CSSProperties
      }
    >
